...
import common.xml.dp.DpInf06205Response;
...
/**
 * @author 2014年1月15日 下午2:08:45<br>
 * 
 *         本来描述：[东莞公交]充值接口
 */
public class INF06205 {
	private static final Log logger = LogFactory.getLog(INF06205.class);

	public static String svcInfName = "INF06205";

	public static String execute(String in0, String in1) {
		DpInf06205Request dpRequest = null;

		DpInf06205Response resp = new DpInf06205Response();
		...
		try {
			...
			dpRequest = new DpInf06205Request(in1);
			...
			// 充值：REG 充值冲正：RGR
			if ("REG".equals(operationType)) {
				ds = SagNoticeManager.rechargeCard(dpRequest, true);
			} else if ("RGR".equals(operationType)) {
				// 先冲正业务后冲正支付
				// 调业务网关
				ds = SagNoticeManager.rechargeCard(dpRequest, false);
			}
	 ...
	 * @throws Exception
	 */
	private static PackageDataSet scs0102reverse(DpInf06205Request dpRequest) throws Exception {
	 ...
	 * @throws Exception
	 *//*
	private static PackageDataSet callSCS0013(DpInf06205Request dpRequest) throws Exception {
		*//**
		 * 调用SCS0001,完成交易操作
		 *//*
		// 通过客户编码查区域编码
		String actionCode = "12010004";// 业务编码
		// 订单受理信息
		IParamGroup g401 = new ParamGroupImpl("401");
		g401.put("4002", dpRequest.getApTransSeq());// 订单编码
		g401.put("4005", "OT001");// 订单类型编码：业务类订单
		g401.put("4144", dpRequest.getChannelCode60To20());// 渠道类型编码
		g401.put("4146", dpRequest.getStaffCode());// 操作员
		g401.put("4142", "OT005");
		g401.put("4012", "东莞通-充值");
		g401.endRow();
		...
		String params = "<PARAMS>" //
				...
				"<LIMITEDAUTHSEQL>" + dpRequest.getLimitedauthseq() + "</LIMITEDAUTHSEQL>" + //
