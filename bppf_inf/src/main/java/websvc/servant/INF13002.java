...
import common.xml.dp.DpInf13002Request;
...
public class INF13002 {

	public static String svcInfName = "INF13002";

	private static final Log logger = LogFactory.getLog(INF13002.class);

	public static String execute(String in0, String in1) {

		DpInf13002Request dpRequest = null;
		RespInfo respInfo = null; // 返回信息头
		
		TInfOperInLog tInfOperInLog = null;
		String svcCode = WebSvcTool.getSvcCode(in0);
		DpInf13002Response resp = null;

		INFLogID id = new INFLogID(svcInfName, TInfDcoperlogDao.PARTY_GROUP_AG);//TODO new group id?

		try {

			dpRequest = new DpInf13002Request(in1);
			...
			rankListQueryRequest.setCustomerId(dpRequest.getCustCode());
			rankListQueryRequest.setInvokeId("INF");
			Map<String,String> scParam = SocketSrvConfig.getParam();
			String key = scParam.get("ODSSIGNKEY");
			key = key==null?"":key;	
			rankListQueryRequest.setSign(SignUtil.getSign(rankListQueryRequest, key));
			String json = JSONObject.fromObject(rankListQueryRequest).toString();
			...
			String rtnString = HttpUtils.queryJsonData("http://"+srvIP+":"+srvPort+"/" + prjName +"/queryRankListPosition", json);
