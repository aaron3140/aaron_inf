...
import common.xml.dp.DpInf05102Request;
import common.xml.dp.DpInf05102Response;
...
public class INF05102 {
	private static final Log log = LogFactory.getLog(INF05102.class);

	public static String svcInfName = "INF05102";
	...
	public static String execute(String in0, String in1) {
		DpInf05102Request dpRequest = null;

		DpInf05102Response resp = new DpInf05102Response();
		...
		try {
			...
			dpRequest = new DpInf05102Request(in1);
			...
			String cardAccNbr = PaymentTool.getTissonCardAcct(dpRequest
					.getCustCode()); // custCode对应的天讯账户号
			...
			log.info("deDuctChannel========" + deDuctChannel + " custCode===="
					+ custcode);
			...
			if (certype.equals("00")
					&& !(cerno.length() == 15 || cerno.length() == 18))
			...
			/**
			 * @description 对于外部传过来的银行编码进行判断
			 * @author lichunan
			 * @date 2014-02-08 11:04
			 */
			TCumAcctDao acctDao = new TCumAcctDao();
			String bankCode = acctDao.getBankCode(dpRequest.getBankAcct());
			// 1、判断银行编码是否为空
			if (!StringUtils.isEmpty(bankCode)) {
				// 2、判断查询出的银行编码和传过来的银行编码是否相等，不相等的话就抛出异常
				if (!bankCode.equals(dpRequest.getBankCode())) {
					throw new INFException(INFErrorDef.INF_BANKCODE_ERROR,
							INFErrorDef.INF_BANKCODE_ERROR_DES);
				}
			}
			...
			for (int i = 0; i < privList.size(); i++) {
				...
				if (dpRequest.getBusiType().equals("BT001")
						&& "ws_ColNPay_Collection_Username".equals(str)) { //
					flag = true;
					break;
				}
				if (dpRequest.getBusiType().equals("BT002")
						&& "ws_ColNPay_Payagent_Username".equals(str)) { //
					flag = true;
					break;
				}
			}
			if (!flag) {
				throw new Exception("没有验证实时代收付权限");
			}
			...
			if (pdl != null && !"".equals(pdl)) {
				...
				// 代收付产品线 且是信用卡
				if (pds.contains("101")
						&& acctDao.getIsCreditCard(dpRequest.getBankAcct())) {
			...
			// 转账预处理
			if (deDuctChannel.equals("D01")
					&& "01".equals(dpRequest.getTransferFlag())) {

				if (dpRequest.getColleCustCode()
						.equals(dpRequest.getCustCode())) {
				...
				// 收款商户关联机构验证
				if (!TCumInfoDao.verifyMerIdCustCode(
						dpRequest.getColleCustCode(), dpRequest.getMerId())) {
			...
			String buscode = TInfOperInLogManager.convetBussType(dpRequest
					.getBusiType());
			isupdate = TInfOperInLogManager.verifyOrder(orderseq, keep, tmnNum,
					svcInfName, buscode);
			try {
				if (deDuctChannel == null || deDuctChannel.equals("D02")) { // 代扣渠道1
					// 代收
					// 15010006
					ds = bt001(dpRequest);
				} else if (deDuctChannel.equals("D01")) { // 代扣渠道2 代收入账 15010009
					if (cardAccNbr == null) {

						throw new INFException(
								INFErrorDef.BANKACCT_NOT_cardAccNbr,
			...
			// String resultMsg = (String) ds.getParamByID("0002", "000").get(
			// 0);
			//
			// // 转账预处理
			// if (deDuctChannel.equals("D01")
			...
			if (deDuctChannel.equals("D01")
				...
				log.info("3..支付成功 更新");
				// 更新充值状态
				tranDao.updateRecSucStat(keep, transSeq, responseCode,
						responseDesc);
			...
			return resp.toXMLStr(respInfo.getReqWebsvrCode(),
					respInfo.getRespType(), respInfo.getKeep(), "SUCCESS",
					responseCode, responseDesc, transSeq, orderseq);
			...
			if (isTran) {
				...
				return ExceptionHandler.toTanXML(new XmlINFException(resp, e,
						respInfo), infId, tranDao);
			} else {
				return ExceptionHandler.toOutOrderXML(new XmlINFException(resp,
						e, respInfo), infId, null, isupdate, tmnNum, orderseq,
						tranDao, isTransFlag);
			}
	 ...
	 * @throws Exception
	 */
	private static PackageDataSet transfer(DpInf05102Request dpRequest,
	...
	public static PackageDataSet bt001(DpInf05102Request dpRequest)
			throws Exception {

		// 一次路由
		TransManage tm = new TransManage();
		PackageDataSet dataSet2 = tm.firstRoute(dpRequest.getCustCode(),
				"000000", dpRequest.getChannelCode(), "15010006",
		...
		// 组成数据包,调用SCS0001接口
		IServiceCall caller = new ServiceCallImpl();
		PackageDataSet ds = caller.call("SCS", "SCS0001", g401, g402, g404,
				g405, g407, g408);
	...
	public static PackageDataSet bt002(DpInf05102Request dpRequest,
			String cardAccNbr) throws Exception {

		// 一次路由
		TransManage tm = new TransManage();
		PackageDataSet dataSet2 = tm.firstRoute(dpRequest.getCustCode(),
				"000000", dpRequest.getChannelCode(), "15010009",
				dpRequest.getMerId(), dpRequest.getTmnNum(),
				dpRequest.getTxnAmount(), "PT1004", dpRequest.getBankCode());
		...
		// g407.put("4047", "1");// 业务单序号
		// g407.put("4051", newActionCode);// 业务编码
		// g407.put("4087", "SCS_ADDRESS");// 联系地址
		// g407.put("4088", addr);//发送机构标识码
		// g407.put("4080", "1");// 控制标识
		// g407.endRow();
		// }
		g407.put("4047", "1");// 业务单序号
