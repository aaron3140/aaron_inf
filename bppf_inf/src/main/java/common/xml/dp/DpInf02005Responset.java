...
public class DpInf02005Responset extends CommonRespAbs{
	...
	private static String [] dataKeys={"CUSTCODE","OBJCODE","OBJNAME","STAT"};
	
	public String toXMLStr(String REQWEBSVRCODE, String RESPONSETYPE,String KEEP, String RESULT, 
			String RESPONSECODE, String RESPONSECONTENT, List transAcctList) {
		...
		if (transAcctList.size() > 0) {
			for (int i = 0,y = transAcctList.size(); i < y; i++) {
				Element datas = dataSet.addElement(TAG_NAME_14_11);
				String value="";
				for (String key:dataKeys) {
					value=(String)((Map)transAcctList.get(i)).get(key);
					if (value==null) {
						value="";
					}
